...
# 1. separate numeric from non-numeric factors
```{r}
...
# 7. Model categorical using Adonis
```{r}
...
# Alpha diversität - phyloseq - not rarefy beforehand!
```{r}
# ## rarefied alpha div <- not the right way
# plot_richness(physeq.gras, measures = c("Chao1", "Shannon", "Simpson"))
# plot_richness(physeq.gras, measures = c("Chao1"))
...
# 8. Make constrained ordination (dbRDA) using significant/ important factors
## ~trt_NNP
```{r}
...
# 7. prepare data to plot ONLY look.us + konz.us
```{r}
...
# 8. Make constrained ordination (dbRDA) using significant/ important factors  -  konz.us + look.us
##  - konz.us ~ trt_NNP
```{r}
OTU2plot <- OTUs[grep("konz.us", labels$site_code),] # if only for one specific - look.us, konz.us
Labels <- labels[grep("konz.us", labels$site_code),] # if only for one specific - look.us, konz.us
