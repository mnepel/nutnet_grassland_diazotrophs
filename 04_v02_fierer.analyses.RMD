...
topN = 50
(physeq.gras)
most_abundant_taxa = sort(taxa_sums(physeq.gras), TRUE)[1:topN]
...
```{r}
(physeq.gras.subs <- subset_samples(physeq.gras, site_code%in%(target)))
  (physeq.gras.subs <- prune_taxa(taxa_sums(physeq.gras.subs) > 0, physeq.gras.subs))

otu.sub <- as.data.frame(t(otu_table(physeq.gras.subs)))
  otu.sub <- otu.sub[order(rowSums(otu.sub),decreasing=T),]
  otu.sub$rowSums <- rowSums(otu.sub)
  write.table(otu.sub, 
              paste("fierer.analyses/differential.OTUs/", target,"OTU.mat", ".", Sys.Date(),".txt", sep = ""), 
...
# # Africa vs. NorthAmerica
# target = "AFvsNA"
# (physeq.gras.subs <- subset_samples(physeq.gras, continent%in%(c("Africa"))))
# (physeq.gras.subs <- subset_samples(physeq.gras, !continent%in%("Europe")))
# (physeq.gras.subs <- subset_samples(physeq.gras.subs, !continent%in%("Australia")))
# sample_data(physeq.gras.subs)

most_abundant_taxa = sort(taxa_sums(physeq.gras.subs), TRUE)[1:topN]
print(most_abundant_taxa)

physeq.gras.subs.topN = prune_taxa(names(most_abundant_taxa), physeq.gras.subs)
# physeq.bsc.Setmerged <- merge_samples(physeq.bsc.rare100, "Set")


physeq.sim <- physeq.gras.subs.topN
  otu.exp <- as.data.frame(otu_table(physeq.sim))
  otu.exp$rowSums <- rowSums(otu.exp)
  write.table(t(otu.exp), 
              paste("fierer.analyses/differential.OTUs/", target,".OTU.mat.", "top", topN, ".", Sys.Date(),".txt", sep = ""), 
              sep="\t", row.names=TRUE, col.names=NA)

pds <- phyloseq_to_deseq2(physeq.sim, ~ trt_NNP)
pds = DESeq(pds)
...
alpha=0.05
  (sigtab <- res[which(res$pvalue < alpha), ]) # only get OTUs with pvalue (not adjusted) < alpha

sigtab <- cbind(as(sigtab,"data.frame"), as(tax_table(physeq.sim)[rownames(sigtab), ], "matrix"))
  head(sigtab,100)
  dim(sigtab)
  write.table(sigtab, 
              paste("fierer.analyses/differential.OTUs/", target,".sigtab.", alpha, ".", topN, ".", Sys.Date(),".txt", sep = ""),
              sep="\t", row.names=TRUE, col.names=NA)
...
(count.subcluster <- table(sigtab$Zehr.Subcluster))
  write.table(count.phylum, paste("fierer.analyses/differential.OTUs/", target,".sigCount.", alpha, ".", "top", topN, ".phylum.", Sys.Date(),".txt", sep = ""), sep="\t", row.names=TRUE, col.names=NA)
  write.table(count.class, paste("fierer.analyses/differential.OTUs/", target,".sigCount.", alpha, ".", "top", topN, ".class.", Sys.Date(),".txt", sep = ""), sep="\t", row.names=TRUE, col.names=NA)
  write.table(count.order, paste("fierer.analyses/differential.OTUs/", target,".sigCount.", alpha, ".", "top", topN, ".order.", Sys.Date(),".txt", sep = ""), sep="\t", row.names=TRUE, col.names=NA)
  write.table(count.family, paste("fierer.analyses/differential.OTUs/", target,".sigCount.", alpha, ".", "top", topN, ".family.", Sys.Date(),".txt", sep = ""), sep="\t", row.names=TRUE, col.names=NA)
  write.table(count.genus, paste("fierer.analyses/differential.OTUs/", target,".sigCount.", alpha, ".", "top", topN, ".genus.", Sys.Date(),".txt", sep = ""), sep="\t", row.names=TRUE, col.names=NA)
  write.table(count.cluster, paste("fierer.analyses/differential.OTUs/", target,".sigCount.", alpha, ".", "top", topN, ".cluster.", Sys.Date(),".txt", sep = ""), sep="\t", row.names=TRUE, col.names=NA)
  write.table(count.subcluster, paste("fierer.analyses/differential.OTUs/", target,".sigCount.", alpha, ".", "top", topN, ".subcluster.", Sys.Date(),".txt", sep = ""), sep="\t", row.names=TRUE, col.names=NA)
